      DOUBLE PRECISION FUNCTION ALPHASMZ(Q,AMZ,NLOOP)
...
c$$$      INTEGER NLOOP,J
c$$$      DOUBLE PRECISION Q,QP,QM,AMZ,CMASS,BMASS,X1,X2,X3,EP,DIFF1,ALPHAS
c$$$      COMMON/QMASS/CMASS,BMASS
c$$$C---     B0=(11.-2.*F/3.)/4./PI
c$$$      DATA B0/0.716197243913527D0,0.66314559621623D0,0.61009394851893D0/
c$$$C---     C1=(102.D0-38.D0/3.D0*F)/4.D0/PI/(11.D0-2.D0/3.D0*F)
c$$$      DATA C1/.565884242104515D0,0.49019722472304D0,0.40134724779695D0/
c$$$C---     C2=(2857.D0/2.D0-5033*F/18.D0+325*F**2/54)
c$$$C---     /16.D0/PI**2/(11.D0-2.D0/3.D0*F)
c$$$      DATA C2/0.453013579178645D0,0.30879037953664D0,0.14942733137107D0/
c$$$C---     DEL=SQRT(4*C2-C1**2)
c$$$
c$$$      X1=ALPHAS(Q,AMZ,1)
c$$$      X2=ALPHAS(Q,AMZ,2)
c$$$      X3=ALPHAS(Q,AMZ,3)
...
c$$$      BETA(J)=-B0(J)*X1**2
c$$$      DIFF1=(ALPHAS(QP,AMZ,1)-ALPHAS(QM,AMZ,1))/4d0/EP/BETA(J)
c$$$      ENDIF
c$$$      IF (NLOOP .EQ.2) THEN 
c$$$      BETA(J)=-B0(J)*X2**2*(1D0+C1(J)*X2)
c$$$      DIFF1=(ALPHAS(QP,AMZ,2)-ALPHAS(QM,AMZ,2))/4d0/EP/BETA(J)
c$$$      ENDIF
c$$$      IF (NLOOP .EQ.3) THEN 
c$$$      BETA(J)=-B0(J)*X3**2*(1D0+C1(J)*X3+C2(J)*X3**2)
c$$$      DIFF1=(ALPHAS(QP,AMZ,3)-ALPHAS(QM,AMZ,3))/4d0/EP/BETA(J)
c$$$      ENDIF
