...
c      SUBROUTINE MGNUPLOT(N,M,BTIT,LTIT,SCALE)
c      IMPLICIT REAL*8 (A-H,O-Z)
c      IMPLICIT INTEGER (I-N)
c      CHARACTER*(*) LTIT,BTIT,SCALE
c      include 'histo.f'
cc--- added these variables to scale plots at intermediate steps
c      logical scaleplots                  
c      double precision scalefac
c      common/scaleplots/scalefac,scaleplots
c
c      istring=LEN_TRIM(TITLE(N))
c
c      IF(BOOK(N).NE.'YES') RETURN
c      WRITE(97,101) TITLE(N)(1:istring), TITLE(N)(1:istring),
c     & TITLE(N)(1:istring), HMIN(N),HMAX(N)
c  101 FORMAT( /1x,                               
c     &' set title ','"',A,' distribution" font "Helvetica, 20"',/1X,
c     &' set xlabel ','"',A,'" font "Helvetica, 20"',/1X,
c     &' set ylabel ','"d{/Symbol s}/d',A,
c     &' [fb]" font "Helvetica, 20"',/1X,
c     &' set xrange [ ',F10.5,':',F10.5,']')
c      if(SCALE .eq. 'log') then
c         write(97,*) ' set logscale y'
c      endif
c      write(97,*) ' plot "-" with histeps'
c
c      DO 1 J=1,NBIN(N)
c      IF(HIST(N,J).EQ.0.) GO TO 1
c      if (scaleplots) then
c      WRITE(97,'(3(2X,G13.6))')  
c     & XHIS(N,J),scalefac*HIST(N,J),HIST(M,J)
c      else
c      WRITE(97,'(3X,G13.6,2(2X,G13.6))')  
...
c      write(97,*) ' plot "-" with histeps'
      ...
      DELTA=XHIS(N,1)-HMIN(N)
      
      WRITE(97,101) TITLE(N)(1:istring),N
  101 FORMAT('# ',A,' index ',I3)
      DO 1 J=1,NBIN(N)
c      IF(HIST(N,J).EQ.0.) GO TO 1
      if (scaleplots) then
      WRITE(97,'(4(1X,G14.8))')  
     & XHIS(N,J)-DELTA,XHIS(N,J)+DELTA,scalefac*HIST(N,J),HIST(M,J)
      else
      WRITE(97,'(4(1X,D14.8))')  
     &          XHIS(N,J)-DELTA,XHIS(N,J)+DELTA,HIST(N,J),HIST(M,J)
      endif
    1 CONTINUE
      WRITE(97,*)
      WRITE(97,*)
